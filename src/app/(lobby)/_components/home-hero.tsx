...
export function HomeHero({ videoUrl }: HomeHeroProps) {
  ...
  return (
    <section
      ...
      aria-label="Presentación BIOMSTEP"
    >
          ...
          <div className="mx-auto flex w-full max-w-7xl items-end justify-between gap-4">
            ...
            {!videoUrl ? (
              <div className="hidden flex-col gap-2 text-[0.7rem] font-medium uppercase tracking-[0.2em] text-white/50 sm:flex">
                <span
                  className={cn(
                    "transition-colors",
                    activePhase === hero.layers.anatomy && "text-white"
                  )}
                >
                  {hero.layers.anatomy}
                </span>
                <span
                  className={cn(
                    "transition-colors",
                    activePhase === hero.layers.insole && "text-primary"
                  )}
                >
                  {hero.layers.insole}
                </span>
                <span
                  className={cn(
                    "transition-colors",
                    activePhase === hero.layers.footwear && "text-white"
                  )}
                >
                  {hero.layers.footwear}
                </span>
            ...
            >
              <span className="text-xs">
                {videoComplete
                  ? "Sigue bajando para ver la tienda"
                  : hero.scrollHint}
              </span>
              <ChevronDownIcon
